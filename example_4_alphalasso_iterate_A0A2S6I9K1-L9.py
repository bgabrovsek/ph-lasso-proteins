...
ph_diagrams = ph_extended_diagrams(d["xyz"]["loop"], d["xyz"][termini], use_cache=True)
    ...
    for fws in range(0,5):
        for a in np.arange(0.01, 1.2, 0.01):
            for m in np.arange(0.01, 1.2, 0.01):
                ...
                tp, fp, fn = find_matchings(measured_data=maxima_ranges, ground_truth=d["deep_" + termini], distance=dist)
                #print(tp, fp, fn)
                if len(peaks[0]) == 9:
                    print("peaks is 9", tp)
                    if tp > max_tp:
                        max_tp = tp
                        print("max_tp", max_tp)
                        #interactive_ph_plot(d, ph_diagrams, termini, f_bottle, f_smooth, peaks)
            #     if len(peaks[0]) == 9 and tp>=8:
